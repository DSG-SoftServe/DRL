...
class Memory:
    ...
    def __stat__(self, mute=False):
        if not mute:
            print "S:", len(self.screens), "A:", len(self.actions), "R:", len(self.rewards), "D:", len(self.D)
            
    def __get_state__(self, number):
        state = self.screens[number - 3 : number + 1]
    ...
    def addStart(self, next_screen):
        self.screens.append(next_screen)
        self.__stat__()
    ...
    def add(self, action, reward, next_screen):       
        ...
        self.screens.append(next_screen)
        # Number of last element in screens list
        screen_num = len(self.screens) - 1
        if len(self.screens) > self.game_step_phase:
    ...
    def state(self):
        return self.__get_state__(len(self.screens) - 1)
